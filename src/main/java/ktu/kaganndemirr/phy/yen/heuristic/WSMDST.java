...
public class WSMDST extends Solver {
    private static final Logger logger = LoggerFactory.getLogger(WSMDST.class.getSimpleName());
    ...
    public Solution solveHWSM(final Graph<Node, GCLEdge> physicalTopology, final List<Application> applications, final Evaluator evaluator, final String wsmNormalization, double wAVB, double wTT, double wLength, double wUtil, int rate, final String mcdmObjective) {
        ...
        if (Objects.equals(mcdmObjective, Constants.AVBTT)){
            solution = MCDMSpecificMethods.WSMLWRDSTAVBTT(avbUnicastCandidates, ttUnicasts, wsmNormalization, wAVB, wTT);
        } else if (Objects.equals(mcdmObjective, Constants.AVBTTLENGTH)) {
            solution = MCDMSpecificMethods.WSMLWRDSTAVBTTLength(avbUnicastCandidates, ttUnicasts, wsmNormalization, wAVB, wTT, wLength);
        } else if (Objects.equals(mcdmObjective, Constants.AVBTTLENGTHUTIL)) {
            solution = MCDMSpecificMethods.WSMLWRDSTAVBTTLengthUtil(avbUnicastCandidates, ttUnicasts, wsmNormalization, wAVB, wTT, wLength, wUtil, rate);
        }
