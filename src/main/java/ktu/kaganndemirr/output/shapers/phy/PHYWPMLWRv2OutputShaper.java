...
import ktu.kaganndemirr.output.holders.phy.PHYWPMLWRHolder;
import ktu.kaganndemirr.output.holders.phy.PHYWPMLWRv2Holder;
...
public class PHYWPMLWRv2OutputShaper {
    private static final Logger logger = LoggerFactory.getLogger(PHYWPMLWRv2OutputShaper.class.getSimpleName());
    ...
    public void writeSolutionToFile(List<Unicast> solution) {
        try {
            BufferedWriter writer = new BufferedWriter(new java.io.FileWriter(Paths.get(topologyOutputLocation, "Routest.txt").toString()));
    ...
    public void writeLinkUtilizationsToFile(List<Unicast> solution, Graph<Node, GCLEdge> graph, int rate) {
        Path resultsPath = Paths.get(mainOutputLocation, "Results.txt");
        try {
            ...
            BufferedWriter mainResultWriter = new BufferedWriter(new java.io.FileWriter(resultsPath.toString(), true));
        ...
        logger.info("Unused Links, Max Loaded Link Number, Max Loaded Link Utilization, Average Link Utilization, Variance and Std written to {}", resultsPath);
    }
